...
/* This module is used by the concurrent unordered map container for memory
 * management. It allocates nodes and reuses them to keep the total memory
 * allocation the program under check. Since the container is concurrent, the
 * allocated nodes are kept in two concurrent queues : active and free The
 * active queue contains nodes available for the unordered map and the free
 * queue contains nodes that have been deleted by the map. Nodes are moved from
 * the free queue to the active queue when the number of available nodes goes to
 * zero*/
 ...
 public:
  ...
  node_type *memory_pool_pop() {
    ...
    while (!active_queue->pop(n)) {
      ...
      node_type *chunk =
          (node_type *)std::malloc(chunk_size * sizeof(node_type));
      assert(chunk != nullptr);
