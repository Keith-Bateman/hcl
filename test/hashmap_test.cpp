...
void hcl_map_operations(struct thread_arg *t) {
  for (int i = 0; i < t->num_operations; i++) {
    ...
    int k = random() % 1000000;
    if (op == 0)
      block_map->LocalInsert(k, k);
    else if (op == 1)
      block_map->LocalFind(k);
    else if (op == 2)
      block_map->LocalErase(k);
  }
}
...
int main(int argc, char *argv[]) {
  ...
  if (size_of_request != array_size) {
    if (my_rank == 0)
      std::cout << " Please set request size and number of RPC threads in "
                   "constants.h. Default value for request size is "
                << TEST_REQUEST_SIZE << " and number of RPC_THREADS is 1"
                << std::endl;
  }
  ...
  if (my_rank == 0)
    std::cout << " Baseline throughput = " << baseline << "reqs/sec"
              << std::endl;
  MPI_Allreduce(&local_map_throughput_l, &local_map_throughput, 1, MPI_DOUBLE,
                MPI_MAX, MPI_COMM_WORLD);

  if (my_rank == 0)
    std::cout << " Local_operations_throughput = " << local_map_throughput
              << " reqs/sec" << std::endl;
  ...
  MPI_Allreduce(&remote_map_throughput_l, &remote_map_throughput, 1, MPI_DOUBLE,
                MPI_MAX, MPI_COMM_WORLD);
  if (my_rank == 0)
    std::cout << " Remote Operations throughput = " << remote_map_throughput
              << " req/sec" << std::endl;
  delete (block_map);
  delete (stl_map);
  MPI_Finalize();
